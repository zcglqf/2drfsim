...
%       CreateExcitationGradientWaveforms.m
%       CreateRephasingGradientWaveformByArea.m  (optional, if you use inter-line rephasing)
...
sys = mr.opts('MaxGrad', 10, 'GradUnit','mT/m', ...
              'MaxSlew', 200, 'SlewUnit','T/m/s');
sysimg = mr.opts('MaxGrad', 30, 'GradUnit','mT/m', ...
              'MaxSlew', 200, 'SlewUnit','T/m/s');
% sys.rfRasterTime = 1e-7;
seq = mr.Sequence(sys);
...
FovM = 50e-3;
FovP = 10e-3;   % meters (40 mm)
FovS = 2e-3;    % meters (5 mm)
FOV  = [FovP, FovS];
FOE_S = 60e-3;              % meters
Offc = [0.02, 0.02]'; % meters
flipAngle = pi/3;   % 90 deg excitation
tiltAngle = 0;
...
for li = 1:nLines
    ...
    if li > 1
        ...
        gLead{1}.channel = 'x';
        gLead{2}.channel = 'y';
        seq.addBlock(gLead{1}, gLead{2});
        anchorTime = anchorTime + mr.calcDuration(gLead{1});
    end
...
kWidth = Nx*deltak;
readoutTime = 6.4e-4;
gx = mr.makeTrapezoid('x',sysimg,'FlatArea',kWidth,'FlatTime',readoutTime);
adc = mr.makeAdc(Nx,sysimg,'Duration',gx.flatTime,'Delay',gx.riseTime);

% Pre-phasing gradients
preTime=8e-4;
%gxPre = mr.makeTrapezoid('x',sysimg,'Area',-gx.area/2-deltak/2,'Duration',preTime);
% gzReph = mr.makeTrapezoid('z',sysimg,'Area',-gz.area/2,'Duration',preTime);
%gyPre = mr.makeTrapezoid('y',sysimg,'Area',-Ny/2*deltak,'Duration',preTime);
% we need no minus for in-plane prephasers because of the spin-echo (position reflection in k-space)
gxPre = mr.makeTrapezoid('x',sysimg,'Area',-gx.area/2 - deltak,'Duration',preTime);
gyPre = mr.makeTrapezoid('y',sysimg,'Area',-Ny/2*deltak,'Duration',preTime);

% Phase blip in shortest possible time
dur = ceil(2*sqrt(deltak/sysimg.maxSlew)/10e-6)*10e-6;
gy = mr.makeTrapezoid('y',sysimg,'Area',deltak,'Duration',dur);

% Refocusing pulse with spoiling gradients
% rf180 = mr.makeBlockPulse(pi,sysimg,'Duration',500e-6,'use','refocusing');
% [rf180, gz180] = mr.makeSincPulse(pi,sysimg,'Duration', 4000e-6,...
%     'SliceThickness',FovS*1.3,'apodization',0.5,'timeBwProduct',4,...
%     'use','refocusing');
% gz180.channel = 'yy';
% gzSpoil = mr.makeTrapezoid('y',sysimg,'Area', 2*6/FovS,'Duration',3*preTime);
...
minTE = (exciFinTime - kspCenTime) + durationToCenter + mr.calcDuration(gxPre);
...
delayTE=TE - (exciFinTime - kspCenTime) - preTime - durationToCenter;
delayTE = round(delayTE / sys.gradRasterTime) * sys.gradRasterTime;

% Define sequence blocks
% seq.addBlock(rf,gz);
if delayTE > sys.gradRasterTime
...
seq.write('epi_gre.seq');   % Output sequence for scanner
