...
function testReturnsCorrectCount(testCase)
    sys = makeTestSys();
    [g1, ~] = ObjCreator.CreateMPSTrapezoidOfOperationalArea(10, sys, [], 0.5);
    [g2, ~] = ObjCreator.CreateMPSTrapezoidOfOperationalArea([10 20], sys, [], 0.5);
    [g3, ~] = ObjCreator.CreateMPSTrapezoidOfOperationalArea([10 20 30], sys, [], 0.5);
...
function testGradientAmplitudeRight(testCase)
    ...
    g = ObjCreator.CreateMPSTrapezoidOfOperationalArea(1000, sys);
    g = g{1};
...
function testRejectsInvalidLength(testCase)
    sys = makeTestSys();
    verifyError(testCase, @() ObjCreator.CreateMPSTrapezoidOfOperationalArea([], sys, [], 0.5), ...
        'CreateMPSTrapezoidOfOperationalArea:invalidLength');
end

function testZeroAreaBranch(testCase)
    sys = makeTestSys();
    [grads, aux] = ObjCreator.CreateMPSTrapezoidOfOperationalArea([0 0], sys, [], 0.5);
...
function testRampTimeRasterization(testCase)
    sys = makeTestSys();
    A = [100 50 0];
    [~, aux] = ObjCreator.CreateMPSTrapezoidOfOperationalArea(A, sys, [], 0.3);
...
function testTriangleVsTrapezoidBranches(testCase)
    ...
    [~, auxTri] = ObjCreator.CreateMPSTrapezoidOfOperationalArea([Atri], sys, [], blankSlopeRatio);
    verifyEqual(testCase, auxTri.plateauDuration, 0);

    % --- Trapezoid case: maxAbsArea large ---
    Atrap = 5.0 * (slopFactor * sys.maxGrad * rampTime);
    [~, auxTrap] = ObjCreator.CreateMPSTrapezoidOfOperationalArea([Atrap], sys, [], blankSlopeRatio);
...
function testAmplitudeMatchesDefinition(testCase)
    ...
    [grads, aux] = ObjCreator.CreateMPSTrapezoidOfOperationalArea(signedOperationalAreas, sys, [], blankSlopeRatio);
...
function testPlateauDurationFormulaMatchesCode(testCase)
    ...
    % Run function
    [~, aux] = ObjCreator.CreateMPSTrapezoidOfOperationalArea(A, sys, [], blankSlopeRatio);
